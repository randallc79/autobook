...
def find_audiobook_folders(root_path):
    try:
        candidates = [os.path.join(root_path, item) for item in os.listdir(root_path)
                      if os.path.isdir(os.path.join(root_path, item)) and
                      sum(1 for f in os.listdir(os.path.join(root_path, item)) if f.lower().endswith('.mp3')) > 1]
        return candidates
    except: return []

parser = argparse.ArgumentParser(description='Audiobook Organizer CLI')
parser.add_argument('--root_path', required=True, help='Input root directory')
...
for folder in tqdm(folders, desc="Processing folders"):
    ...
    if asin:
        try:
            proc = subprocess.Popen(['m4b-merge', '-i', folder, '-o', output_path], stdin=subprocess.PIPE, text=True)
            proc.communicate(input=asin + '\n')
            if proc.returncode == 0: success = True
        except: pass
    if not success:
        try:
            subprocess.run(['python', m4binder_path, '--mode', 'single', '--input-folder', folder,
                            '--output-file', output_file, '--metadata-source', 'openlibrary',
                            '--title', title, '--author', author], check=True)
            success = True
        except: pass
    if success:
